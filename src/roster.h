...
// "Abriss" (dropped off) is detected automatically -- no GPS needed.
#pragma once
...
struct PeerInfo {
  ...
  int16_t rssiDbm; // fast-smoothed (EMA), the value the SCHWACH logic judges
...
// drives the periodic ABRISS reminder in the UI.
bool anyDroppedOff();
...
// Most recent roster-level event (join / falling back / Abriss / back), for
// the idle screen's "last event" line. Returns nullptr if none yet.
const char *lastEventText();
uint32_t lastEventTimestampMs();

// Timestamp of the most recent SCHWACH/ABRISS event specifically (a subset of
// the above -- DA/ZURUECK don't count). Used by the UI to wake the display for
// the two events actually worth interrupting the rider for. 0 if none yet.
uint32_t lastAlertTimestampMs();

// Bumped on every SCHWACH/ABRISS event. The UI's dedup key: unlike the
// timestamp above it also distinguishes two alerts logged in the very same
// millisecond (e.g. two peers dropping in one tick()). 0 = no alert yet.
